...
#ifdef RUNNER
// Endless-runner: reconstruct the eye ray from the real camera matrices instead
// of the GetRay() Shadertoy camera, so shading matches the free WASD/IJKL view.
...
void main() {
    ...
    if (t < 0.0) {
        ...
        #ifdef RUNNER
            // Dissolve the terrain into the sky toward the rim of the patch so the
            // finite plane reads as an endless landscape. The patch spans
            // [-0.5, 0.5], so distance-to-rim is max(|x|,|z|) / 0.5.
            float rim = max(abs(pos.x), abs(pos.z)) * 2.0;
            color = mix(color, fogColor, smoothstep(0.7, 0.99, rim));
        #endif
    }
